...
@Component({
  ...
  template: `<div class="modal-header">
              <div class="modal-title">绑定API</div>
            </div>
            <nz-transfer
            [nzDataSource]="list"
...
export class RoleBindAPIResourceComponent implements OnInit {
  userId = '';
  list: any[] = [];
  role: any = {};
  update: false;
  ...
  change(ret: any ): void {
    ...
    if (ret.from === 'right') {
      ...
      // const resourceIds = ret.list.map((item: any) => item.key);
      post(this.http, url, { resources: ret.list, type: 'urn:api'}).subscribe(result => {
  ...
  ngOnInit(): void {
    ...
    this.service.getOwnerAPIResource(this.role.name).subscribe((data: any) => {
      this.list = data;
    });
